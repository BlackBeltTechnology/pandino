...
const defaultExternalsMapping = {
  '@pandino/react-dom': '@pandino/react-dom',
  'react/jsx-runtime': '@pandino/react-dom/react/jsx-runtime',
  'react': '@pandino/react-dom/react',
  'react-router-dom': '@pandino/react-router-dom/react-router-dom',
};
...
const pandinoExternalizeReact = ({
  ...
  return {
    name: 'rollup-plugin-react-externalize',
    options: (opts) => ({
      ...opts,
    ...
    generateBundle: async (options, bundle, isWrite) => {
      ...
      generatedExternals.forEach(e => externals.set(e.token, e.identifier));
      externalRefs.forEach(e => externals.set(e.token, e.identifier));
      const externalFinalArray = [...externals.entries()].map(([token, identifier]) => ({ token, identifier }));
      const result = template(componentsMap, externalFinalArray, codes, mockProcess);
